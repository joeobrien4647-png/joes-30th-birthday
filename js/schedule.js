...
function initActivitySignups() {
    ...
    const savedSignups = Store.get('activitySignups', {});
    const userName = Store.getRaw('activityUserName') || '';
    ...
    buttons.forEach(btn => {
        ...
        btn.addEventListener('click', function() {
            ...
            if (!name) {
                ...
                if (!name) return;
                Store.setRaw('activityUserName', name);
            }
            ...
            Store.set('activitySignups', savedSignups);
            updateActivityDisplay(activity);
        });
    });
    ...
    function updateActivityDisplay(activity) {
        ...
        if (attendeesEl) {
            attendeesEl.innerHTML = savedSignups[activity]
                ? savedSignups[activity].map(name => `<span class="attendee-tag">${escapeHtml(name)}</span>`).join('')
                : '';
        }
    }
}
...
function initItineraryCompare() {
    ...
    // Voting
    const votes = Store.get('itinVotes', { chill: 0, adventure: 0, balanced: 0 });
    let userVote = Store.getRaw('itinUserVote');
    ...
    voteButtons.forEach(btn => {
        btn.addEventListener('click', function() {
            ...
            if (userVote === choice) {
                ...
                votes[choice]++;
                Store.setRaw('itinUserVote', choice);
                userVote = choice;
            }

            Store.set('itinVotes', votes);
            updateVoteDisplay();
        });
    });

    function updateVoteDisplay() {
        const currentVote = Store.getRaw('itinUserVote');
...
function initDailyHighlights() {
    ...
    let highlights = Store.get('dailyHighlights', {});
    let votes = Store.get('highlightVotes', {});
    ...
    form.addEventListener('submit', function(e) {
        ...
        Store.set('dailyHighlights', highlights);
        Store.set('highlightVotes', votes);
    ...
    function updateWinner() {
        ...
        if (winner && maxVotes > 0) {
            if (winnerDisplay) {
                ...
                document.getElementById('winner-text').textContent = `"${winner.text}" - ${winner.author} (${maxVotes} stars)`;
            }
        } else {
            if (winnerDisplay) winnerDisplay.style.display = 'none';
        }
